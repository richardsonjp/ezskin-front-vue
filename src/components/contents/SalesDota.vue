<template>
  <b-container fluid style="padding-left:40px;padding-right:40px;">
    <b-col>
      ...
      <b-pagination align="right" :total-rows=" pagination.totalRows" :per-page="pagination.perPage" v-model="pagination.currentPage" class="my-0" @change="fetchSalesDota()" />
    </b-col>
    <b-row>
      <b-col cols="3">
...
export default {
  ...
  mounted: function() {
    // console.log('isi filteran gamess',this.dotaFilter)
    // if (this.dotaFilter) {
      this.fetchSalesDota(this.dotaFilter)
    // }
  },
  methods: {
    ...
    sharePagination: function(obj) {
      this.$emit('pagination-count-SalesDota', obj)
    },
    onSubmit: function (e) {
      e.preventDefault()
    ...
    fetchSalesDota: function (dotaFilter) {
      this.filters.gameId = dotaFilter || this.dotaFilter
      setTimeout(() => {
